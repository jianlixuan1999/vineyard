...
function simulate(agent::MyQLearningAgentModel, environment::T, startstate::Tuple{Int,Int}, maxsteps::Int;
    ...
    for _ ∈ 1:maxsteps
        ...
        if rand() < ϵ
            # exploration            
            a = isempty(available_actions) ? -1 : rand(available_actions)  # -1 or some flag for no action
        else
            # exploitation
        ...
        # # debug
        # println("Step: $step, State: $s, Action: $a, Available Actions: $available_actions")
